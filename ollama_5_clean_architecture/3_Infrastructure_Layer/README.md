# Infrastructure Layer  | 編寫實作

## 實作 :

- 在[infrastructure下](./infrastructure/ollama_adapter.py)建立`OllamaSummarizer`包含url、model_name
- 覆蓋`AISummarizer`的`summarize`方法，pyload、`response.json().get('response')`
- 記得`summarize`要使用Summary格式回傳
